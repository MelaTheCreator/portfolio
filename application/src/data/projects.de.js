export const projectsDE = [
  {
    title: "Lernplattform für Grundschulkinder",
    description:
      "Diese Lernplattform entstand mit dem Ziel, kindgerechte Inhalte klar zu strukturieren und intuitiv zugänglich zu machen. Der Fokus lag auf verständlicher Nutzerführung, einfacher Navigation und einer ruhigen, übersichtlichen Gestaltung.",
    learnings:
      "Konzeption nutzerzentrierter Interfaces, Arbeit mit React-Komponenten, strukturierter Aufbau von UI-Logik.",
    tech: ["React", "Tailwind CSS", "JavaScript"],
  },
  {
    title: "Taro-Karten Webanwendung",
    description:
      "Eine kreative Webanwendung aus einer frühen Lernphase, bei der der Fokus auf ausgefallenem, detailreichem Design und spielerischer Nutzerinteraktion lag. Die Anwendung besteht aus mehreren HTML-Seiten und nutzt umfangreiches natives CSS zur Gestaltung.",
    learnings:
      "Arbeit mit mehreren HTML-Seiten, intensiver Einsatz von nativem CSS, Verständnis für Layout, Styling, Selektoren und visuelle Effekte sowie erste Erfahrungen mit strukturiertem Frontend-Aufbau.",
    tech: ["HTML", "CSS", "JavaScript"],
    link: "",
  },
  {
    title: "Schlaupause – Lernplattform für Kinder",
    description:
      "Schlaupause ist ein gemeinsames Projekt, das in Zusammenarbeit mit einer Freundin entsteht. Ziel ist es, Kindern einen spielerischen und gleichzeitig strukturierten Zugang zu Lerninhalten zu ermöglichen. Besonderer Wert wird auf eine klare Navigation, ruhige Gestaltung und altersgerechte Nutzerführung gelegt.",
    learnings:
      "Arbeit im Team, gemeinsame Konzeption von Features, Abstimmung von UI-Entscheidungen sowie Umsetzung mit React in kollaborativer Entwicklungsarbeit.",
    tech: ["React", "Tailwind CSS", "JavaScript"],
    link: "https://github.com/imkehoegden/schlaupause",
  },
];
